        ...
        <div class="login-box" style=" background-image: url(content/images/backgroud_crud.png);">
            ...
            <form class="login-register" method="post">
                <input class="form-control" type="text" placeholder="Họ Và Tên" name="fullname"
                    value="<?=$query['fullname']?>" style="width: 900px; height:45px"><span style="color:white; font-size: 17px "><?php echo "<i>$fullname_error</i>"; echo '</br>';?></span>
                <input class="form-control" type="text" placeholder="Tài Khoản..." name="username"
                    value="<?=$query['username']?>" style="width: 900px; height:45px"><span style="color:white; font-size: 17px "><?php echo "<i>$username_error</i>"; echo '</br>';?></span>
                <input class="form-control" type="password" placeholder="Mật Khẩu..." name="password"
                    value="<?=$query['password']?>" style="width: 900px; height:45px"><span style="color:white; font-size: 17px "><?php echo "<i>$password_error</i>"; echo '</br>';?></span>
                <input class="form-control" type="text" placeholder="Email" name="email" value="<?=$query['Email']?>"
                    style="width: 900px; height:45px"><span style="color:white; font-size: 17px "><?php echo "<i>$email_error</i>"; echo '</br>';?></span>
                <input class="form-control" type="text" placeholder="Số điện thoại" name="phone"
                    value="<?=$query['phone']?>" style="width: 900px; height:45px"><span style="color:white; font-size: 17px "><?php echo "<i>$phone_error</i>"; echo '</br>';?></span>
                <input class="form-control" type="text" placeholder="Địa chỉ" name="address"
                    value="<?=$query['address']?>" style="width: 900px; height:45px"><span style="color:white; font-size: 17px "><?php echo "<i>$address_error</i>"; echo '</br>';?></span>
                <div class="pq_ns">
                    <div class="col-md-8">
                        <select class="form-select" name="permissions" required aria-label=".form-select-sm example">
                            <option value="admin" <?php if ($query['permissions'] == 'admin') {echo 'selected';} ?>>
                                Admin</option>
                            <option value="user" <?php if ($query['permissions'] == 'user') {echo 'selected';} ?>>User
                            </option>
                        </select>
                    </div>
                </div>
                <div class="btn-submit">
                    ...
                    <button class="btn btn-success" name="capnhat">Thay Đổi</button>
                </div>
            </form>
        </div>
    </div>
